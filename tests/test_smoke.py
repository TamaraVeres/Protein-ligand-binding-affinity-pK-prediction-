...
def test_pocket_edges_rebuild_exactly(graphs):
    """The slimmed cache stores no pocket edges; they must rebuild bit-for-bit."""
    _, poc = graphs
...
def test_similar_pairs_matches_rdkit():
    """Vectorised similarity must agree with RDKit, which defined the original split."""
...
def test_clusters_contain_all_similar_pairs():
    """Single-linkage guarantee: no pair over the cutoff may span two clusters."""
...
def test_evaluate_handles_trailing_batch_of_one(graphs, n):
    """A final batch of size 1 used to collapse y to 0-d and break torch.cat."""
    lig, poc = graphs
    ckpt = torch.load(CHECKPOINT, map_location="cpu", weights_only=False)
    model = AffinityModel(**ckpt["model_config"])
    model.load_state_dict(ckpt["model_state"], strict=True)
...
def test_predict_cli_reports_every_row(tmp_path):
    """Every input row must appear in the output, including failures."""
...
def _synthetic_dataset(tmp_path, n=120, n_families=12):
    """A dataset of n ligands drawn from n_families distinct chemotypes.

    Members of a family share most bits, so a working similarity split must keep each
    family entirely on one side of the train/test boundary.
    """
    import csv as _csv
...
def test_similarity_split_is_not_sequential_and_has_no_leakage(tmp_path):
    """Regression: this split used to silently degrade into a sequential one."""
...
def test_similarity_split_raises_when_fingerprints_missing(tmp_path):
    """It must fail loudly rather than quietly returning a sequential split."""
    from src.training.trainer import split_dataset_by_similarity
    from src.data.fingerprints import DEFAULT_INDEX_NAME
